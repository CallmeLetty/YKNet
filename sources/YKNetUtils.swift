...
class ArFileHandler {
    func generateFilePath(cover: Bool = true,
        ...
        guard FileManager.default.fileExists(atPath: fileLocation) else {
            // 目标文件不存在
            throw YKNetError(type: .valueNil("fileLocation:\(fileLocation)"))
        }
        ...
        guard subArr.count > 0,
              let fileName = subArr.last else {
                  // 目标文件路径有误
                  throw YKNetError(type: .invalidParameter("directoryPath:\(directoryPath)"))
              }
        ...
        if !FileManager.default.fileExists(atPath: directoryPath),
           let url = URL(string: directoryPath) {
            do {
                ...
                // 创建文件夹失败
                throw YKNetError(type: .invalidParameter("directoryPath:\(directoryPath),error:\(error)"))
            }
        }
        ...
        if FileManager.default.fileExists(atPath: path) {
            if cover,
               let fileUrl = URL(string: path) {
                ...
                // 文件存在，不覆盖，抛错
                throw YKNetError(type: .fileExists(path))
            }
        }
        return path
    }
    ...
    func copyFile(filePath: String,
        ...
        do {
            ...
            throw YKNetError(type: .copyFile(filePath,
                                          targetPath),
                          code: err.code,
...
extension URLRequest {
    ...
    // TODO: not work
    mutating func makeBody(httpMethod: YKNetHTTPMethod,
...
//            let bodyData = bodyStr.data(using: .utf8)
            ...
            throw YKNetError(type: .serialization("params"))
        }
...
extension Data {
    func json() throws -> [String: Any] {
        ...
        guard let dic = object as? [String: Any] else {
            throw YKNetError.convert("Any", "[String: Any]")
        }
